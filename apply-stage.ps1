...
function CreateStage15 {
    Write-Host "📦 Stage 15: Add JSDoc documentation for all methods..." -ForegroundColor Yellow
...
"@ | Out-File -FilePath "scripts/add-jsdoc.cjs" -Encoding utf8

    # Запускаем скрипт
    node scripts/add-jsdoc.cjs

    git add .
    git commit -m "docs: add JSDoc documentation for all methods"
    git push origin master

    Write-Host "✅ Stage 15 complete!" -ForegroundColor Green
    Write-Host ""
    Write-Host "📌 JSDoc documentation added to all methods!" -ForegroundColor Yellow
}
switch ($Stage) {
    "0" { CreateStage0 }
    "1" { CreateStage1 }
    ...
    default {
        ...
        Write-Host "Available: 0, 1, 2, 3, 4, 5, 6, 6b, 6c, 7, 8, 9, 91, 92, 93, 94, 95, 10, 11, 12, 13, 14, 15" -ForegroundColor Yellow
    }
}